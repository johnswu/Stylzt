...
//------------ hooks and filters -------------------------------
	...
	add_filter("ProjectTheme_get_regular_post_project_rows", 'projectTheme_get_post_main_function_rows', 0, 1);
...
function wsl_alter_hook_provider_icon_markup_function($provider_id) {
	?>
			<div class="row<?php if ($provider_id == 'Facebook') echo ' mg-btm' ?>">
				<div class="col-md-12">
					<a rel="nofollow" href="javascript:void(0);" class="wsl_connect_with_provider btn btn-block btn-<?php echo strtolower($provider_id) ?>" data-provider="<?php echo $provider_id ?>">
...
function ProjectTheme_get_users_links()
{
	...
	<ul class="nav navbar-nav">
		<li class="title"><a href="#" onclick="toggleSubNav('MyAccountSubMenu');return false;"><i class="fa fa-caret-right" id="MyAccountSubMenuCaret"></i>&nbsp;&nbsp;<?php _e("My Account Menu",'ProjectTheme'); ?></a></li>
		<ul class="subnav" id="MyAccountSubMenu">
			...
			<li class="last"><a href="<?php echo wp_logout_url(); ?>"><?php echo __("Log Out",'ProjectTheme'); ?></a></li>
			...
			?>
	<ul class="nav navbar-nav">
        <li class="title"><a href="#" onclick="toggleSubNav('HiringSubMenu');return false;"><i class="fa fa-caret-right" id="HiringSubMenuCaret"></i>&nbsp;&nbsp;<?php _e("Service Contractor Menu",'ProjectTheme'); ?></a></li>
			<?php
		...
		<ul class="subnav" id="HiringSubMenu">
			...
			?>
	<ul class="nav navbar-nav">
        <li class="title"><a href="#" onclick="toggleSubNav('FindWorkSubMenu');return false;"><i class="fa fa-caret-right" id="FindWorkSubMenuCaret"></i>&nbsp;&nbsp;<?php _e("Service Provider Menu",'ProjectTheme'); ?></a></li>
			<?php
...
{
	$c = "<select name='projectss' class='form-control' onchange='on_proj_sel();' id='my_proj_sel'><option value='0'>".__('Select','ProjectTheme')."</option>";
	global $wpdb;
...
function projectTheme_get_post_rows($arr = '')
{
	do_action('ProjectTheme_get_regular_post_project_rows', $arr);	
}
            ...
            <div class="card film art">
              ...
              <div class="card-footer">
                <ul class="list-inline filters">
                  <li><?php echo get_the_term_list( get_the_ID(), 'project_cat', '', ', ', '' ); ?></li>
                </ul>
              </div><!-- end card-footer -->
            </div><!-- end card -->
					...
					<tr>
						...
						<td>
							...
							if($projecttheme_escrow_was_made_for_project_not_released == false):
								...
								$projectTheme_project_function_filter_1 = true;
								$projectTheme_project_function_filter_1 = apply_filters('projectTheme_project_function_filter_1', $pid);

								if($projectTheme_project_function_filter_1 == true):
							...
							<?php 
								...
								echo sprintf(__('Escrow was made. <a href="%s">Go and release it.</a>','ProjectTheme'), $kk); ?>	
...
function ProjectTheme_send_email_subscription($pid)
{
	...
	$opt = get_post_meta($pid,'subscription_email_sent',true);
	
	if(empty($opt))
	{
		update_post_meta($pid,'subscription_email_sent',"111a");
...
function ProjectTheme_send_email_posted_project_not_approved_admin($pid)
{
	...
	$opt = get_post_meta($pid, 'ProjectTheme_send_email_posted_project_not_approved_admin', true);
	
	if($enable != "no" and empty($opt)):
	
		update_post_meta($pid, 'ProjectTheme_send_email_posted_project_not_approved_admin', '1');
		$post 			= get_post($pid);
